...
@Component
@Slf4j
public class  RedisCacheUtilities<T> {
    @Resource
    RedisTemplate<Object, Object> redisTemplate;
    private static final long DEFAULT_CACHE_LIEV_TO_TIME = 7200;
    ...
    public void delPattern(String  key) {
        if (StringUtils.isNotBlank(key)) {
            ...
            Set<Object> keys=redisTemplate.keys(patternKey);
     ...
     * @return
     */
    public boolean addToList(String key, T value) {
        try {
            return addToList(key,value,DEFAULT_CACHE_LIEV_TO_TIME);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
     ...
     * @return
     */
    public boolean addToList(String key, T value, long time) {
        try {
     ...
     * @return
     */
    public boolean addToList(String key, List<Object> value, long time) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            if (time > 0) expire(key, time);
            return true;
    ...
    public long removeList(String key,Object value) {
        try {
            Long remove = removeList(key,1,value);
            return remove;
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
